...
var select_year = '';
var select_month = '';
var select_query = 'entry.tbl.php';
...
function set_datetime_active()
{
    ...
    select_query = 'entry.tbl.php?year='+year+'&month='+month;    
    select_year = year;
    select_month = month;
    ...
    for (var i=(year-1); i<=(year+1); i++) {
        ...
        jQuery(id).bind("click", function(){
            ...
            var query = 'entry.tbl.php?year='+select_year;
            set_grid_url(query);
            return false;
        });
    }    
    ...
    for (var i = 1; i <= 12; i++) {
        ...
        jQuery(id).bind("click", function(){
            ...
            select_month = select;            
            // set the grid query url
            var query = 'entry.tbl.php?year='+select_year+'&month='+select_month;
            set_grid_url(query);
            return false;
        });
    }    
...
function post_delete() {
    ...
    $("#tblobj").jqGrid('delGridRow',
        id, { url: 'entry_delete.php', reloadAfterSubmit: false}
    );
}
...
$(document).ready(function () {
    ...
    $("#tblobj").jqGrid({
        ...
        sortorder: "asc",
        loadonce: false,
        editurl: "entry_save.php",
        ondblClickRow: function(rowid) {
            //$(this).jqGrid('editGridRow', rowid); 
            window.parent.add_entry_tab(rowid);
        }
    });
