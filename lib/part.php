...
{      
    ...
    {
        $db = GLOBALDB();
        $sql = 'SELECT name, alias, round(price,1) AS price, quantity FROM parts WHERE partno = "TAIHO" order by id asc';
    ...
    // 
    // $cid: company id, used to search the company's parts
    ...
    function search($key, $cid)
    {
        ...
        if ($result = $db->query($sql)) {
            ...
            while($rows) {
                ...
                $price = $this->get_sale_price($cid, $pid);
