...
{      
    ...
    function restore($sign)
    {
        ...
        // 变量sign在+的时候不会出错,在-的时候可能出错,因为库存的数量可能一减为负数!
        // 要提示用户有库存问题(入库单修改的时候).
        $num = count($parts);
        for ($i = 0; $i < $num; $i++) 
        {
            ...
            if(sign == '-') 
            {
                $result = $this->restore_purchase($pid,$quantity);
                if ($result == false)
                    return $result;
                    
                $result = $this->restore_entry($pid,$quantity);
                if ($result == false)
                    return $result;

                $result = $this->restore_presale($pid,$quantity);
                if ($result == false)
                    return $result;
            }
            // 要insert sql前做的
            ...
            {            
                $result = $this->restore_sale($pid,$quantity);
                if ($result == false)
                    return $result;
            }
        }
    ...
    function restore_purchase($pid, $quantity)
    {        
    ...
    function restore_entry($pid, $quantity)
    {        
    ...
    function restore_presale($pid, $quantity)
    {        
        // 对价格单, 没操作quantity字段, 不需要回归
        $result = true;
        return $result;
    }

    function restore_sale($pid, $quantity)
    {        
